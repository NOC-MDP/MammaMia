...
from mamma_mia import Campaign, inventory
...
print(
    f"sensors available for platform Slocum G2: {inventory.list_sensors(platform_model='Slocum_G2')}"
)
print(f"sensors of type CTD: {inventory.list_sensors(sensor_type='CTD')}")
print(
    f"sensor info: {inventory.get_sensor_info(platform_model='APEX', sensor_type='CTD')}"
)

print("<=========> starting Mamma Mia AUV Campaign test run <===========>")
# create campaign
campaign = Campaign(
    name="RAPID array virtual mooring",
    description="single slocum glider deployment at a RAPID mooring",
    verbose=True,
)
# %%
# create platform entity (mutable)
Churchill = inventory.create_platform_entity(
    entity_name="Churchill", platform_model="Slocum_G2", serial_number="unit_398"
)
...
# # # add mission
campaign.add_mission(
    mission_name="RAD24_01",
