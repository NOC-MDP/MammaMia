...
def interpolate(
    ...
    """
    returns interpolated values from input coords, input coords can be:
        - single floats/datetime strings
        - list floats/datetime strings
        - numpy arrays of float64 and np.datetime[ns]
    coords are assumed to have the following keys:
        {
            "latitude": [26.834],
            "longitude": [-15.142],
            "depth": [25.0],
...
def create_interpolator(mission: xr.DataTree = None, stores: dict = None):
    """
    returns an pyinterp 4D interpolator coverings all the data stores that have been downloaded.
    function uses store attributes from a mission with a trajectory or from a bare stores
    dict that has been generated by a simluator integration.
    If a data source needs regridding, then it be as part of this function
    """
    logger.info(
