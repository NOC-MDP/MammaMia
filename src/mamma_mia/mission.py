...
from mamma_mia.gsw_funcs import ConvertedP, ConvertedTSP
from mamma_mia.interpolator import Interpolators
from mamma_mia.sim_error import simulate_sensor_error
from mamma_mia.trajectory import NavigationKeys, Trajectory
from mamma_mia.worlds import SourceConfig, WorldExtent, WorldsAttributes, WorldsConf
...
class Mission:
    ...
    def for_campaign(
        ...
        nav_keys = NavigationKeys.from_datalogger(
            datalogger=platform.attrs.sensors[data_logger_key],
            platform_attrs=platform.attrs,
        )
        ...
        surfaced_mask = (
            trajectory.depth[:] < platform_attributes.near_surface_thresh
        ) & (np.abs(dz) < platform_attributes.ascent_thresh)
        event[surfaced_mask] = "surfaced"
        platform.behaviour = np.array(event, dtype="U8")

        if platform.attrs.NEMA_coordinate_conversion:
            logger.info("Platform requires NEMA coordinate conversion")
            for i in range(trajectory.longitude.__len__()):
                trajectory.longitude[i] = cls.__convert_to_decimal(
                    trajectory.longitude[i]
                )
            for i in range(trajectory.latitude.__len__()):
                trajectory.latitude[i] = cls.__convert_to_decimal(
                    trajectory.latitude[i]
                )
            logger.info(
                "Successfully converted from NEMA coordinates to decimal degrees"
            )
        ...
        geospatial_attrs = GeospatialAttributes(
            ...
            geospatial_lat_units=cls.get_parameter_units(
                platform_attrs=platform.attrs, parameter="latitude"
            ),
            geospatial_lon_max=np.max(trajectory.longitude),
            geospatial_lon_min=np.min(trajectory.longitude),
            geospatial_lon_units=cls.get_parameter_units(
                platform_attrs=platform.attrs, parameter="longitude"
            ),
            ...
            geospatial_bounds=(
                f"POLYGON(({np.min(trajectory.longitude)},"
            ...
            time_coverage_start=str(
                np.datetime_as_string(trajectory.time[0], unit="s")
            ),
        ...
        extent = WorldExtent(
            ...
            time_start=str(
                np.datetime_as_string(
                    trajectory.time[0] - np.timedelta64(30, "D"), unit="D"
                )
            ),
            time_end=str(
                np.datetime_as_string(
                    trajectory.time[-1] + np.timedelta64(30, "D"), unit="D"
                )
            ),
        ...
        worlds = WorldsConf(
            attributes=WorldsAttributes(
                extent=extent, matched_worlds={}, interpolator_priorities={}
            ),
            worlds={},
            stores={},
        )
        ...
        mission_total_time_seconds = (trajectory.time[-1] - trajectory.time[0]).astype(
            "timedelta64[s]"
        )
        mission_total_time_steps = np.ceil(
            mission_total_time_seconds.astype(int) / mission_time_step
        ).astype(int)
        for name, sensor in platform.attrs.sensors.items():
            for name2, specification in sensor.specification.items():
                payload[name2] = np.empty(
                    shape=mission_total_time_steps, dtype=np.float64
                )
        return cls(
            platform=platform,
            attrs=attrs,
    ...
    @staticmethod
    def get_parameter_units(
        platform_attrs, parameter: str, instrument_type: str = "data_logger"
    ) -> str:
        sensor_key = None
    ...
    def build_mission(self, cat: Cats):
        ...
        matched_worlds.search_worlds(
            ...
            extent=self.worlds.attributes.extent,
            source=self.attrs.source_config,
        )
        self.worlds.attributes.matched_worlds = matched_worlds.entries
        data_stores = get_worlds(
            cat=cat, worlds=self.worlds, source=self.attrs.source_config
        )
    ...
    def fly(self, interpolator: Interpolators):
        ...
        """
        logger.info(
            f"flying {self.attrs.mission} using {self.platform.attrs.entity_name}"
        )
        ...
        resampled_flight = self._resample_flight(
            flight=flight, new_interval_seconds=self.attrs.mission_time_step
        )
        # subset flight to only what is needed for interpolation (position rather than orientation)
        flight_subset = {
            key: resampled_flight[key]
            for key in ["longitude", "latitude", "depth", "time"]
        }
        navigation_alias = {}
        ...
        for k1, v1 in self.platform.attrs.sensors.items():
            if self.platform.attrs.sensors[k1].instrument_type == "data_logger":
                navigation_keys = list(
                    self.platform.attrs.sensors[k1].specification.keys()
                )
                for k2, parameter in self.platform.attrs.sensors[
                    k1
                ].specification.items():
                    for nav_key in navigation_keys:
                        if nav_key == parameter["meta_data"].parameter_id:
                            navigation_alias[nav_key] = parameter[
                                "meta_data"
                            ].alternate_labels
        marked_keys = []
        for key in self.payload.keys():
            try:
                logger.info(
                    f"flying through {key} world and creating interpolated data for flight"
                )
                ...
                # get aliases incase key doesn't match
                    ...
                    if track is None:
                        logger.warning(
                            f"no interpolator found for parameter {key} marking parameter for removal from payload"
                        )
            ...
            if key != "TIME" and self.attrs.apply_obs_error:
                ...
                for k3, sensor in self.platform.attrs.sensors.items():
                    if key in sensor.specification.keys():
                        sensor_key = k3
                        break
                track = simulate_sensor_error(
                    model_t=track,
                    mission_ts=self.attrs.mission_time_step,
                    accuracy_bias=self.platform.attrs.sensors[sensor_key].specification[
                        key
                    ]["accuracy"],
                    resolution=self.platform.attrs.sensors[sensor_key].specification[
                        key
                    ]["resolution"],
                    drift_per_month=self.platform.attrs.sensors[
                        sensor_key
                    ].specification[key]["drift_per_month"],
                    m_min=self.platform.attrs.sensors[sensor_key].specification[key][
                        "range"
                    ][0],
                    m_max=self.platform.attrs.sensors[sensor_key].specification[key][
                        "range"
                    ][1],
                    percent_errors=self.platform.attrs.sensors[
                        sensor_key
                    ].specification[key]["percent_errors"],
                    noise_std=self.platform.attrs.sensors[sensor_key].specification[
                        key
                    ]["noise_std"],
                )
        ...
        for world in self.worlds.attributes.matched_worlds.values():
            for alt_key, alt_parameter in world.alternative_parameter.items():
                if alt_parameter is not None:
                    logger.info(
                        f"alternative parameter field in world attributes is not None, {alt_key} requires conversion from {alt_parameter}"
                    )
                    for k1, v1 in self.platform.attrs.sensors.items():
                        if (
                            alt_key
                            in self.platform.attrs.sensors[k1].specification.keys()
                        ):
    ...
    def __convert_parameters(self, conversion_to_apply, flight):
        ...
        if (
            "CONSERVATIVE_TEMPERATURE" in what_we_have
            and "ABSOLUTE_SALINITY" in what_we_have
        ):
            # need to convert from CT and AS
            if (
                "INSITU_TEMPERATURE" in what_we_need
                and "PRACTICAL_SALINITY" in what_we_need
            ):
                converted_tsp = ConvertedTSP.as_ct_2_it_ps(
                    absolute_salinity=self.payload["PRACTICAL_SALINITY"][:],
            ...
            if "INSITU_TEMPERATURE" in what_we_need:
                converted_tsp = ConvertedTSP.ps_pt_2_it_ps(
                    practical_salinity=self.payload["PRACTICAL_SALINITY"][:],
            ...
            logger.error(
                f"unable to convert alternative parameters {conversion_to_apply}"
            )
            return
    ...
    def _resample_flight(flight, new_interval_seconds):
        ...
        if old_interval_seconds > new_interval_seconds:
            logger.warning(
                "resampling to a higher resolution flight, this could result in unrealistic flight behavior"
            )
        ...
        new_time_seconds = np.arange(
            time_seconds[0], time_seconds[-1], new_interval_seconds
        )
        new_time = flight["time"][0] + new_time_seconds.astype("timedelta64[s]")

        # Interpolators
        lon_interp = interp1d(
            time_seconds, flight["longitude"], kind="linear", fill_value="extrapolate"
        )
        lat_interp = interp1d(
            time_seconds, flight["latitude"], kind="linear", fill_value="extrapolate"
        )
        depth_interp = interp1d(
            time_seconds, flight["depth"], kind="linear", fill_value="extrapolate"
        )
        ...
        for key in ["pitch", "yaw", "roll"]:
            if key in flight:
                interp_func = interp1d(
                    time_seconds, flight[key], kind="linear", fill_value="extrapolate"
                )
    ...
    def show_payload(self, parameter: str = None, in_app: bool = False):
        ...
        if parameter is None:
            ...
            marker = {
                "size": 2,
                "color": np.array(
                    self.payload[initial_parameter][:]
            ...
            if (
                self.platform.attrs.platform_model == "Slocum_G2"
                or self.platform.attrs.platform_model == "Slocum_G2_NonNMEA"
            ):
                latitude = "LATITUDE"
                longitude = "LONGITUDE"
                depth = "GLIDER_DEPTH"
                ...
                raise Exception(
                    f"unsupported platform {self.platform.attrs.platform_model} for payload plotting"
                )
            ...
            parameters[parameter] = {
                "cmin": np.nanmin(self.payload[parameter][:]),
            ...
            if (
                ...
                raise Exception(
                    f"unsupported platform {self.platform.attrs.platform_type} for payload plotting"
                )
    ...
    def plot_trajectory(
        ...
        fig = go.Figure(
            data=[
                go.Scatter3d(
                    x=self.trajectory.longitude,
                    y=self.trajectory.latitude,
                    z=self.trajectory.depth,
                    mode="markers",
                    marker=marker,
                )
            ]
    ...
    def export_as_zarr(self, out_dir: str = None, store: zarr.storage = None):
        ...
        mission.attrs.update(
            {"geospatial_attributes": unstructure(self.geospatial_attrs)}
        )
        ...
        # write platform data
        platform.create_array(
            name="behaviour", shape=self.platform.behaviour.__len__(), dtype="string"
        )
        ...
        trajectory.create_array(
            name="time", data=self.trajectory.time.astype("datetime64[s]")
        )
